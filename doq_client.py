...
def aes_encrypt(key, plaintext):
    ...
    auth_tag = encryptor.tag #16-byte authentication tagpstream-host 10.230.3.93
    return iv + ciphertext + auth_tag 

def aes_decrypt(key, data):
    iv = data[:12] #12 byte GCM IV
    ciphertext = data[12:-16] #everything between IV and AuthTag is ciphertext 
    auth_tag = data[-16:] #the last 16 bytes is AuthTag
...
class DnsClientProtocol(QuicConnectionProtocol):
    def __init__(self, *args,server_pubkey=None, upstream_host=None, upstream_port=None,csv_writer=None,**kwargs) -> None:
    ...
    async def query(self, query_name: str, query_type: str) -> bytes:
        ...
        host_bytes = self.upstream_host.encode()
        host_len = len(host_bytes)

        data = (struct.pack("!B", host_len) +host_bytes +struct.pack("!H", self.upstream_port) + struct.pack("!H", len(encrypted_aes_key)) +encrypted_aes_key + encrypted_payload)
        ...
        try:
            ...
            if answer.q:
                requested_domain = query_name.rstrip('.') #Normalize both domain names by removing trailing dots
                response_domain = answer.q.qname.idna().rstrip('.')
                
                if response_domain != requested_domain:
                    raise ValueError(
                        f"Domain name mismatch! Requested '{requested_domain}', "
                        f"but response contains '{response_domain}'"
                    )
                else:
                    print(f"Domain name verified: {requested_domain}")
        ...
        print("{:<30} {:<10.6f}".format("Wait for response", t4-t3))
        print("{:<30} {:<10.6f}".format("Decrypting response and verifying nonce and domain", t5-t4))
        print("-------------------------------")
        if self.csv_writer:
            self.csv_writer.writerow([query_name,f"{t2-t1:.6f}",f"{t4-t3:.6f}",f"{t5-t4:.6f}",f"{t5-t3+t2-t1:.6f}"])
        return dns_bytes
...
async def main(
    ...
    if timing_log_file:
        ...
        try:
            ...
            if file_is_empty: #write title row only if file is empty.
                csv_writer.writerow(['Domain queried','Query_Packet_Creation_s','Wait_for_Encrypted_Response_s','Response_Decryption_and_Verification_s','Total_Time_s'])
    ...
    async with connect(
        ...
        create_protocol=lambda *a, **kw: DnsClientProtocol(*a, server_pubkey=server_pubkey,csv_writer=csv_writer, upstream_host=args.upstream_host,
    upstream_port=args.upstream_port,**kw),
    ) as client:
        client = cast(DnsClientProtocol, client)
        
        for i in range(count):
            logger.info(f"Sending encrypted DNS query run {i + 1}/{count} for domain {query_name}")
            answer_bytes = await client.query(query_name, query_type)
            try:
                answer = DNSRecord.parse(answer_bytes)
                if i == 0: #log the full DNS answer once to avoid spamming the log
                    logger.info("Received DNS answer (first run only)\n%s" % answer)
            except Exception as e:
                logger.error("Failed to parse DNS answer: %s", e)
            
            #100ms small delay between runs for stability
            if i < count - 1:
