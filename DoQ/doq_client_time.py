...
class DnsClientProtocol(QuicConnectionProtocol):
    ...
    async def query(self, query_name: str, query_type: str) -> bytes:
        ...
        try:
            answer = DNSRecord.parse(dns_bytes) #verify domain name.
            if answer.q:
        ...
        print("{:<30} {:<10.6f}".format("Total time", t5-t3+t2-t1))
        print("-------------------------------")
        if self.csv_writer:
    ...
    if timing_log_file:
        ...
        try:
            ...
            if file_is_empty: #write title row only if file is empty.
                csv_writer.writerow(['Domain queried','Query_Packet_Creation_s','Wait_for_Encrypted_Response_s','Response_Decryption_s','Total_Time_s'])
