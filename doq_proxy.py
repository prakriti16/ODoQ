...
class ProxyDnsClientProtocol(QuicConnectionProtocol):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.client_timing_data = {} #store for client-side timing
    ...
    async def query(self, dns_query_data: bytes) -> DNSRecord:
        ...
        t3=time.time() #this is t_setup_end
        print("Proxy(clientside) forwards query data:", data_with_length)
        t4=time.time() #this is t_setup_start for transmission
        ...
        t2=time.time() #this is t_transmission_start_wait
        ...
        t5=time.time() #end timing for wait and receive
        t_setup=(t3-t1)+(t2-t4) #client timing proxy to upstream
        t_wait=t5-t2
...
class ProxyDnsServerProtocol(QuicConnectionProtocol):
    ...
    async def _forward_query(self, query_data: bytes) -> bytes:
        ...
        upstream_client_proto_timing = {} #client-side timing
        ...
        try:
            ...
            async with connect(
                ...
                upstream_client_proto_timing = upstream_client_proto.client_timing_data #capture the client-side timing breakdown
    ...
    def quic_event_received(self, event: QuicEvent):
        ...
        if isinstance(event, StreamDataReceived) and event.end_stream:
            ...
            try:
                ...
                print("Proxy(serverside) receives encrypted query1 :", event.data) #forward the full data
    ...
    async def _handle_client_query(self, client_stream_id: int, raw_client_data: bytes, t_receive_start: float):
        ...
        upstream_response = await self._forward_query(raw_client_data) #forward the entire raw client payload to upstream
        t_forward_end = time.time()
        ...
        if self.csv_writer:
            try:
                self.csv_writer.writerow([
                    "ENCRYPTED_QUERY", #we can't extract the domain at proxy so just log a placeholder
