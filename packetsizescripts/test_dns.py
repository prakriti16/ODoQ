...
LOG_PATTERN = re.compile(
    r"^\[.*?\]\s+127\.0\.0\.1\s+([^\s]+)\s+(" + "|".join(DNS_RECORD_TYPES) + r")\s+PASS\s+(\d+)ms" #regex for domain name and log detail
)
...
def parse_latest_log_times(domain: str):
    ...
    for line in reversed(lines):
        ...
        if match:
            log_domain, qtype, time_ms_str = match.groups() #domain, query type, time            
...
def parse_all_log_times_to_csv(domain: str) -> None:
                ...
                if match:
                    ...
                    if log_domain == domain and qtype in DNS_RECORD_TYPES:
                        record = {
                            ...
                            'RawLine': line.strip() #include full line for context
                        }
    ...
    if not all_records:
        print(f"WARNING: No records found for domain '{domain}' with types {DNS_RECORD_TYPES}.")
        return
...
def calculate_total_time(times: dict):
    ...
    total_time = T_A #if needed can be modified to add CNAME, NX and other times too.
    return total_time
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("--- FINAL RESOLUTION TIME SUMMARY (A) ---")
    print("="*70)
    print("Domain               | A Time          ")
    print("-" * 70)
