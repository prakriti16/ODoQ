...
def forward_dns_query(dns_query: bytes, resolver: str, port: int = 53) -> bytes:
    """
    Forward DNS query to upstream resolver using UDP.
    """
    ...
    try:
        ...
        logger.error(f"Timeout querying upstream resolver {resolver}")
        raise
    except Exception as e:
        logger.error(f"Error querying upstream resolver: {e}")
        raise
    finally:
        sock.close()
...
class HttpServerProtocol(QuicConnectionProtocol):
    ...
    def http_event_received(self, event: H3Event) -> None:
        if isinstance(event, HeadersReceived) and event.stream_id not in self._handlers:
            ...
            if path.startswith("/dns-query"):
                ...
                return
            elif method == "CONNECT" and protocol == "websocket":
    ...
    async def handle_doh_request(
        ...
        """
        Handle DNS over HTTPS request (RFC 8484).
        """
        stream_id = event.stream_id
        dns_query = None
        ...
        try:
            ...
            self._http.send_headers(
                ...
                headers=[
                    ...
                    (b"content-length", str(len(dns_response)).encode()),
                    (b"server", SERVER_NAME.encode()),
                ],
            )
            self._http.send_data(
            ...
            self._http.send_data(
                stream_id=stream_id,
                data=f"Internal server error: {str(e)}".encode(),
                end_stream=True,
            )
            self.transmit()
...
async def main(
    ...
    def create_protocol(*args, **kwargs):
        return HttpServerProtocol(*args, resolver=resolver, **kwargs)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="8.8.8.8",
        help="Upstream DNS resolver to use (default: 8.8.8.8)",
    )
    parser.add_argument(
